function barPlotWithStat1(ind,x,y,data,ylab,tit,rankXY,fn,pathAnal,colorSel,labels,yrange)
    if(nargin == 11)
        yrange = [];
    end
...
%     set(h,'MarkerSize',6,'LineWidth',1,'Color',[0.7 0.7 0.7]);
    ...
    if(isempty(yrange))
        ...
        elseif((max(data(:)) < 0))
            set(gca,'fontSize',12,'YLim',[1.1*min(x-y) max(x+y)])
        end
    else
        set(gca,'fontSize',12,'YLim',yrange)
    end 
          
    fileName1 = fullfile(pathAnal, fn);
